...
# ── Environment ────────────────────────────────────────────────────────────────
# dotenv is only used locally — on Render, env vars are injected directly.
# We import it optionally so the app doesn't crash if python-dotenv isn't
# installed in a minimal production image.
try:
...
# Allow all origins — Node backend and frontend both call this service
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
ALL_SKILLS = [s for skills in SKILLS_CATEGORIES.values() for s in skills]
...
# Use Tuple instead of tuple[] — compatible with Python 3.9 and 3.11
...
def extract_experience(text: str):
    ...
    if matches:
        ...
        if years >= 7:  return int(years), 20
        if years >= 4:  return int(years), 17
        if years >= 2:  return int(years), 13
        return int(years), 10

    fresher_signals = [
        "fresher", "intern", "trainee", "student", "final year",
...
def tfidf_match_score(resume_text: str, job_desc: str):
...
def get_missing_from_jd(resume_text: str, job_desc: str):
    resume_words = set(clean_text(resume_text).split()) - STOP_WORDS
    jd_words     = set(clean_text(job_desc).split())    - STOP_WORDS
...
def generate_suggestions(
    ...
    high_value = [
        ...
        "react", "typescript", "system design", "microservices",
    ]
...
def rule_based_summary(
    ...
    exp_part   = f"{exp_years} years of experience" if exp_years and exp_years > 0 else "a fresher"
    top_cats   = list(skill_by_cat.keys())[:3]
    skill_part = f"with skills spanning {', '.join(top_cats)}" if top_cats else "with a foundational skill set"
    top_skills = found_skills[:5]
    tech_part  = f"Key technologies include: {', '.join(top_skills)}." if top_skills else ""
    return f"Candidate is {exp_part} at the {edu_level} level, {skill_part}. {tech_part}".strip()
...
def run_analysis(resume_text: str, job_description: str = "") -> dict:
    ...
    suggestions = generate_suggestions(
        skill_data["found"], jd_missing, edu_level, exp_years, job_match, skill_data
    )
...
async def analyze_pdf(
    ...
    if not resume_text or len(resume_text.strip()) < 50:
        raise HTTPException(
            status_code=422,
            detail="Could not extract text. Make sure it is not a scanned image PDF.",
        )
